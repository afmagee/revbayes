...
const MemberRules& Move_EllipticalSliceSamplingSimple::getParameterRules(void) const
{
    ...
    if ( !rules_set )
    {
        ...
        move_member_rules.push_back( new ArgumentRule( "forceAccept"  , RlBoolean::getClassTypeSpec(),         "If we hit maximum slice iterations, should we force the sampler to return the initial values (accept a step of size 0)? Not recommended.", ArgumentRule::BY_VALUE    , ArgumentRule::ANY, new RlBoolean( false ) ) );
